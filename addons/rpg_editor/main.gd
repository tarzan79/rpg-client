...
var panel
var sqlite_class = load("res://addons/rpg_editor/sqlite.gd")
var sqlite
var config = {
    "sqlite": {
        "bdd": "res://database/database.db",
        "driver": "res://lib/gdsqlite.gdns"
       }
   }

func _enter_tree():
    sqlite = load("res://addons/rpg_editor/sqlite.gd").new(config)
    _create_database_and_table()
    main_panel_instance = MainPanel.instance()
    get_editor_interface().get_editor_viewport().add_child(main_panel_instance)
    make_visible(false)
    var item_editor = load("res://addons/rpg_editor/module/item/item_editor.gd").new(sqlite)
    main_panel_instance.get_node("TabContainer/Objet").add_child(item_editor.dock_item)
    
    var type_editor = load("res://addons/rpg_editor/module/type/type_editor.gd").new(config)
    main_panel_instance.get_node("TabContainer/Type").add_child(type_editor.dock_type)
    
    var option_editor = load("res://addons/rpg_editor/module/option/option_editor.gd").new(config)
